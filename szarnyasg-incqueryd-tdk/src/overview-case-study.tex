...
The \textit{RouteSensor} query looks for \textit{Sensor}s that are connected to a \textit{Switch}, but the sensor and the switch are \emph{not} connected to the same \textit{Route}. The graphical representation of the RouteSensor query is shown on \figref{routesensor-pattern}. The RouteSensor query  IQPL (\autoref{subsec:eiq}).\footnote{Note that the two queries are slightly different: the SPARQL query returns only a set of sensors, while the IQPL query returns a set of (Sensor, Switch, SwitchPosition, Route) tuples. TODO explain.}
