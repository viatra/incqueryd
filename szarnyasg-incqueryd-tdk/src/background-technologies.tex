...
% API.
%To access the capabilities of the core, an easy-to-use, type safe API is defined. Using the API, EMF resources and object hierarchies can be loaded and queried incrementally. In addition certain extensions -- such as the validation framework -- can be attached (\autoref{fig:incquery-architecture}). 
% Tooling.
...
\iqd{} requires a distributed, asynchronous messaging framework. For this purpose, we used the Akka framework.
