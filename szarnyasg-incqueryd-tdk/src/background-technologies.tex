...
\pic{graph-classes}{Different graph data models~(based on \cite{DBLP:journals/corr/abs-1006-2361})}
...
\picTiny{cassandra-ring-partitions}{Cassandra's ring for data partitioning \cite{CassandraPartitioning}}

%\pic{cassandra-vnodes}{Cassandra's ring for data partitioning}
% \emph{virtual nodes}
...
\picBig{4s-cluster}{4store's distributed architecture~\cite{harris20094store}}
...
%\pic{akka-actor-path}{Determining an actor's path in Akka}
...
\pic{akka-remote-deployment}{Deploying a remote actor in Akka}
...
%  \begin{figure}
%  \centering
%  \includegraphics[width=0.7\textwidth]{ecore}
%  \caption{A simplified subset of the Ecore metamodel}
%  \label{fig:ecorekernel}
%  \end{figure}
...
\picTiny{ecore-kernel}{A simplified subset of the Ecore metamodel}
...
This problem is well-known in the Java community. There are alternative Java Virtual Machines (JVMs) with specialized Garbage Collectors, like Azul Systems' JVM. However, the Azul JVM is a proprietary product and has specific hardware requirements. 

Instead of vertical scaling, we decided to opt for horizontal scaling. As described in \autoref{nosql}, distributed non-relational databases have been gaining momentum in the last years. For persisting models, we inspected graph databases, but found that their query layer does not scale well in a distributed environment. This is a serious problem for MDE, where the queries are typically more complex than in traditional, transactional database management.

Therefore, we decided to implement our own distributed, scalable query layer, \iqd{}.
