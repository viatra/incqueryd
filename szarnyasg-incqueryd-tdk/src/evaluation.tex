...
\pic{benchmark-original}{The \tb{}'s scenario}%. The green arrows and components are specific to the incremental scenario.}
...
\pic{trainBenchmark_Memory_User_RouteSensor}{\tb{}: memory consumption of the tools}
...
\figref{benchmark/neo/BatchTrafo_RouteSensor} shows that the Neo4j-based incremental tool consistently outperforms the Neo4j's own query engine.

\pic{benchmark/neo/BatchTrafo_RouteSensor}{Total execution times for 50 validations}

\figref{benchmark/neo/OnTheFly_RouteSensor} shows that while the \iqd{} with neo4j is about two orders of magnitude faster than Neo4j's own query engine.
