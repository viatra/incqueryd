...
\pic{trainbenchmark-sequence}{The \tb{}'s sequence}
...
\pic{trainBenchmark_IncQuery_RouteSensor}{\tb{}: reponse times for incremental query evaluation, measured on a single node \cite{rath-high-performance-queries}}
...
\figref{trainBenchmark_IncQuery_RouteSensor} shows the incremental transformation and validation time for the \emph{RouteSensor} query, discussed in \autoref{railroad-system}. The results clearly show the advantage of incremental query engines. Both Eclipse OCL Impact Analyzer and \eiq{} scale very well (their characteristic is almost constant to the model size and linear to the size of the result set), while non-incremental tools scale linearly at best, which renders them inefficient for lange models. %The results show that relational database management systems (MySQL) are , followed by in-memory SPARQL engines
...
\pic{trainBenchmark_Memory_User_RouteSensor}{\tb{}: memory consumption of the tools \cite{rath-high-performance-queries}}

\figref{trainBenchmark_Memory_User_RouteSensor} shows the memory consumption of the diffent tools. It is apparent that incremental tools space--time tradeoff causes them to consume more memory.
