...
% see http://components.neo4j.org/neo4j/snapshot/apidocs/org/neo4j/graphdb/PropertyContainer.html

\subsubsection{Mapping Ecore}

Our concept to reuse \eiq{} for building \iqd{} required us to map EMF's metamodel, Ecore to the domain of property graphs and RDF models.
...
\hline
\bf Ecore concept & 
\bf Property graph concept &
\bf RDF concept \tabularnewline \hline\hline

\texttt{EClass} instance     & nodes' \texttt{type} property & \texttt{rdfs:Resource} \\ \hline
\texttt{EAttribute} instance & nodes' property names         & \texttt{rdf:Property}  \\ \hline
\texttt{EReference} instance & edge label                    & \texttt{rdf:Property}  \\ \hline
\texttt{EDataType} instance  & Java primitive types          & \texttt{rdfs:Datatype} \\ \hline
...
A possible mapping between from Ecore to property graphs and RDF/RDFS models are show on \autoref{tab:ecore-mapping}.

For an example, take the railroad system case study (introduced in \autoref{railroad-system}, metamodel shown on \figref{trainbenchmark-metamodel}, instance model shown on \figref{neoclipse-graph}).
\begin{itemize}
  \item \texttt{Segment} is an \texttt{EClass} instance. In the property graph, types cannot be represented explicitly. Instead, for each node representing a \texttt{Segment} instance, we add a \texttt{type} property with the value \texttt{Segment}.
  \item \texttt{Segment\_length} is an \texttt{EAttribute} instance. For each graph node representing a \texttt{Segment}, we define a property with the value \texttt{Segment\_length}.
  \item \texttt{TrackElement\_Sensor} is an \texttt{EReference} instance. For each edge representing a \texttt{TrackElement\_Sensor} instance, we add the \texttt{TRACKELEMENT\_SENSOR} label.
  \item \texttt{EInt} in an \texttt{EDataType} instance. Each attribute with this type (e.g.\ \texttt{Segment\_length}) are defined with the Java primitive type \texttt{int}.
\end{itemize}
Note that the property graph and the RDF data models lack some advanced features of Ecore. For example, an Ecore \texttt{EReference} specifies a relationship between \texttt{EClass} instances and defines multiplicity constraints and containment. 
