  ...
  def runTest() {
    for (i <- 1 to 99) {
      if (i % 2 == 0) {
        ...
        MQTTPublisher.publishResults(createPatternId(TEST_PATTERN11, TEST_QUERY1), resultSet12)
        MQTTPublisher.publishResults(createPatternId(TEST_PATTERN12, TEST_QUERY1), resultSet11)
        MQTTPublisher.publishResults(createPatternId(TEST_PATTERN21, TEST_QUERY2), resultSet22)
        MQTTPublisher.publishResults(createPatternId(TEST_PATTERN22, TEST_QUERY2), resultSet21)
      }
      Thread.sleep(1000)
    }
  }
