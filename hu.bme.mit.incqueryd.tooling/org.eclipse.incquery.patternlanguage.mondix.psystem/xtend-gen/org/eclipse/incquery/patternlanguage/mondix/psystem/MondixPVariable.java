...
public class MondixPVariable {
  public static PVariable toPVariable(final ValueReference it, final PBody pBody, final MondixPModel model) {
    ...
    if (!_matched) {
      if (it instanceof IntValue) {
        ...
        _switchResult = MondixPVariable.convertLiteralValue(Integer.valueOf(_value), pBody);
      }
    }
    if (!_matched) {
      if (it instanceof DoubleValue) {
        ...
        _switchResult = MondixPVariable.convertLiteralValue(Double.valueOf(_value), pBody);
      }
    }
    if (!_matched) {
      if (it instanceof BoolValue) {
        ...
        _switchResult = MondixPVariable.convertLiteralValue(Boolean.valueOf(_isValue), pBody);
      }
    }
    if (!_matched) {
      if (it instanceof StringValue) {
        ...
        _switchResult = MondixPVariable.convertLiteralValue(_value, pBody);
      }
    }
    if (!_matched) {
      if (it instanceof VariableValue) {
        _matched=true;
        _switchResult = MondixPVariable.convertVariableValue(((VariableValue)it), pBody);
      }
    }
    if (!_matched) {
      if (it instanceof AggregatedValue) {
        _matched=true;
        _switchResult = MondixPVariable.convertAggregatedValue(((AggregatedValue)it), pBody, model);
      }
    }
    if (!_matched) {
  ...
  public static PVariable convertAggregatedValue(final AggregatedValue aggregatedValue, final PBody pBody, final MondixPModel model) {
