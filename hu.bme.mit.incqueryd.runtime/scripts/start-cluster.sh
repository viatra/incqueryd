...
export YARN_RM_IP=$(docker inspect --format="{{.NetworkSettings.IPAddress}}" $YARN_RM)
export YARN_NM1_IP=$(docker inspect --format="{{.NetworkSettings.IPAddress}}" $YARN_NM1)
export YARN_NM2_IP=$(docker inspect --format="{{.NetworkSettings.IPAddress}}" $YARN_NM2)
