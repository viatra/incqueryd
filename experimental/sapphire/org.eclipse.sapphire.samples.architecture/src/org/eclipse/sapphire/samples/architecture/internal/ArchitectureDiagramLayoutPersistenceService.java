...
public class ArchitectureDiagramLayoutPersistenceService extends
	...
	protected void init() {
		super.init();
		this.architecture = (ArchitectureSketch) context(
				SapphireDiagramEditorPagePart.class).getLocalModelElement();
	...
	private void read(DiagramNodePart nodePart) {
		...
		if (!component.disposed()) {
			...
			if (this.nodeBounds.containsKey(nodeId)
					&& this.nodeBounds.get(nodeId) != null) {
	...
	private void write(DiagramNodePart nodePart) {
		...
		if (!component.disposed()) {
			if (isNodeLayoutChanged(nodePart)) {
				this.architecture.detach(this.componentListener,
						"/Components/Position/*");
				writeComponentBounds(component, nodePart);
				this.architecture.attach(this.componentListener,
						"/Components/Position/*");
			}
	...
	public DiagramConnectionInfo read(DiagramConnectionPart connPart) {
		...
		if (this.connectionBendPoints.containsKey(key)) {
			DiagramConnectionInfo connectionInfo = new DiagramConnectionInfo(
					this.connectionBendPoints.get(key));
	...
	private void write(DiagramConnectionPart connPart) {
		ComponentDependency dependency = (ComponentDependency) connPart
				.getLocalModelElement();
		if (!dependency.disposed()) {
			if (isConnectionLayoutChanged(connPart)) {
				this.architecture.detach(this.componentDependencyListener,
						"/Components/Dependencies/ConnectionBendpoints/*");
				writeDependencyBendPoints(dependency, connPart);
				this.architecture.attach(this.componentDependencyListener,
						"/Components/Dependencies/ConnectionBendpoints/*");
			}
	...
	public void dispose() {
		if (this.diagramPartListener != null) {
			context(SapphireDiagramEditorPagePart.class).detach(
					this.diagramPartListener);
		}
	...
	private void load() {
		context(SapphireDiagramEditorPagePart.class).setGridVisible(
				this.architecture.isShowGrid().content());
		context(SapphireDiagramEditorPagePart.class).setShowGuides(
				this.architecture.isShowGuides().content());

		ElementList<Component> components = this.architecture.getComponents();
		ConnectionService connService = context(
				SapphireDiagramEditorPagePart.class).service(
				ConnectionService.class);
		for (Component component : components) {
			DiagramNodePart nodePart = context(
					SapphireDiagramEditorPagePart.class).getDiagramNodePart(
					component);
			if (nodePart != null) {
				DiagramNodeBounds bounds = null;
				bounds = new DiagramNodeBounds(component.getPosition().getX()
						.content(), component.getPosition().getY().content(),
						-1, -1, false, false);
				nodePart.setNodeBounds(bounds);

				// load the embedded connection layout
				ElementList<ComponentDependency> dependencies = component
						.getDependencies();

				for (ComponentDependency dependency : dependencies) {
					DiagramConnectionPart connPart = getConnectionPart(
							connService, dependency);
					if (connPart != null) {
						ElementList<ConnectionBendpoint> bendpoints = dependency
								.getConnectionBendpoints();
						if (bendpoints.size() > 0) {
							int index = 0;
							for (ConnectionBendpoint bendpoint : bendpoints) {
								connPart.addBendpoint(index++, bendpoint.getX()
										.content(), bendpoint.getY().content());
							}
						}
					}
				}
	...
	private void handleNodeLayoutChange(Component component) {
		DiagramNodePart nodePart = context(SapphireDiagramEditorPagePart.class)
				.getDiagramNodePart(component);
		DiagramNodeBounds nodeBounds = new DiagramNodeBounds(component
				.getPosition().getX().content(), component.getPosition().getY()
				.content());
		nodePart.setNodeBounds(nodeBounds);
	}

	private void handleConnectionBendpointChange(
			ComponentDependency componentDependency) {
		ConnectionService connService = context(
				SapphireDiagramEditorPagePart.class).service(
				ConnectionService.class);
		DiagramConnectionPart connPart = getConnectionPart(connService,
				componentDependency);
		if (connPart != null) {
			List<Point> bendpoints = new ArrayList<Point>();
			for (ConnectionBendpoint bendpoint : componentDependency
					.getConnectionBendpoints()) {
				bendpoints.add(new Point(bendpoint.getX().content(), bendpoint
						.getY().content()));
			}
	...
	private void addDiagramPartListener() {
		...
		context(SapphireDiagramEditorPagePart.class).attach(
				this.diagramPartListener);
	}
	...
	private void handleDiagramPageEvent(DiagramPageEvent event) {
		SapphireDiagramEditorPagePart diagramPart = (SapphireDiagramEditorPagePart) event
				.getPart();
	...
	private void addModelListeners() {
		this.componentListener = new FilteredListener<PropertyEvent>() {
			...
			protected void handleTypedEvent(final PropertyEvent event) {
				Component component = event.property().element()
						.nearest(Component.class);
				if (component != null) {
		...
		this.componentDependencyListener = new FilteredListener<PropertyEvent>() {
			@Override
			protected void handleTypedEvent(final PropertyEvent event) {
				ComponentDependency componentDependency = event.property()
						.element().nearest(ComponentDependency.class);
	...
	private void writeDependencyBendPoints(ComponentDependency dependency,
			DiagramConnectionPart connPart) {
		final ElementList<ConnectionBendpoint> bpInModelList = dependency
				.getConnectionBendpoints();
	...
	private void doSave() {
		...
		// bend points that
		// are calculated using connection router, we don't modify the
		// corresponding model properties
		// in order to allow "revert" in source editor to work correctly.
		// So we need to do an explicit save of the node bounds and connection
		// bend points here.
		this.architecture.detach(this.componentListener,
				"/Components/Position/*");
		this.architecture.detach(this.componentDependencyListener,
		...
		for (DiagramNodePart nodePart : context(
				SapphireDiagramEditorPagePart.class).getNodes()) {
			Component component = (Component) nodePart.getLocalModelElement();
			if (!component.disposed()) {
				writeComponentBounds(component, nodePart);
			}
		...
		ConnectionService connService = diagramPart
		...
		if (this.connectionBendPoints.containsKey(key)) {
			...
			if (bendpoints.size() != oldBendpoints.size()) {
				...
				for (int i = 0; i < bendpoints.size(); i++) {
					...
					if (newPt.getX() != oldPt.getX()
							|| newPt.getY() != oldPt.getY()) {
						changed = true;
						break;
					}
				}
	...
	private boolean isDiagramLayoutChanged() {
		...
		if (!context(SapphireDiagramEditorPagePart.class).disposed()) {
			...
			for (DiagramNodePart nodePart : context(
					SapphireDiagramEditorPagePart.class).getNodes()) {
				if (!nodePart.getLocalModelElement().disposed()
						&& isNodeLayoutChanged(nodePart)) {
					changed = true;
					break;
				}
			}
			for (DiagramConnectionPart connPart : connService.list()) {
				if (!connPart.getLocalModelElement().disposed()
	...
	private DiagramConnectionPart getConnectionPart(
			ConnectionService connService, Element element) {
		for (DiagramConnectionPart connPart : connService.list()) {
