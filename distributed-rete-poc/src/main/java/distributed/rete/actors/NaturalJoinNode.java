...
public class NaturalJoinNode extends AbstractJoinNode {
	...
	protected UpdateMessage joinNewTuples(Collection<Tuple> newTuples, JoinSide joinSide, UpdateType updateType) {
		...
		// System.out.println("[" + getSelf().path() + "] Join side is: " +
		// joinSide);
		// System.out.println("[" + getSelf().path() + "] Tuples added: " +
		// newTuples.size());
		...
		// System.out.println(existingTuplesIndexer.getMap());
		...
		for (Tuple newTuple : newTuples) {
			...
			// System.out.println(extractedTuple);
			...
			for (Tuple matchingTuple : matchingTuples) {
				...
				result.add(tuple);
				// n++;
				// System.out.println("n = " + n + ", result size = " +
				// result.size());
			}
