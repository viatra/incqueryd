...
public class Neo4jClient {
	...
	public Collection<Map<String, Object>> runCyperQuery(String query) {
		final RestCypherQueryEngine queryEngine = new RestCypherQueryEngine(restApi);
		final Collection<Map<String, Object>> result = IteratorUtil.asCollection(queryEngine.query(query, MapUtil.map()));
		return result;
	}

	public Collection<Object> runGremlinQuery(String query) {
		final RestGremlinQueryEngine queryEngine = new RestGremlinQueryEngine(restApi);
		final Collection<Object> result = IteratorUtil.asCollection(queryEngine.query(query, MapUtil.map()));
		return result;
