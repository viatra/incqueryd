...
/**
 * TermEvaluatorNode [...] deserves special mention because it diverges significantly from the classic RETE concept. It evaluates a GTASM expression on tuples
 * and filters those tuples for which it evaluates to true. It is similar to an alpha node, with one key difference: the filtering condition is not required to
 * be constant. The filtering condition is an arbitrary GTASM term, it is considered as a black box.
 * 
